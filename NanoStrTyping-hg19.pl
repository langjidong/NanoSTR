...
while(<IN>)
{
    ...
    for(my $i=0;$i<$step_size;$i++)
    {
	    ...
	    `perl $Bin/script/Seed_sequence.pl $Bin/database-hg19/by_chr/$tmp[0].fa $tmp_start $step_size $tmp[3] ./Seed/$tmp[3].tmp1`;
	    `perl $Bin/script/Seed_sequence.pl $Bin/database-hg19/by_chr/$tmp[0].fa $tmp_end $step_size $tmp[3] ./Seed/$tmp[3].tmp2`;
...
`cat $configure|awk '{print \$1\"\\t\"\$2-500\"\\t\"\$3-\$2+1+1000\"\\t\"\$4}'|while read a b c d;do perl $Bin/script/Seed_sequence.pl $Bin/database-hg19/by_chr/\${a}.fa \${b} \${c} reference Blast/\${d}/reference.fa;done`;
